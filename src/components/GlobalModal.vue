  ...
  <modal-base ref="modalBaseRef" :active="active" :destroyed="destroy">
    <div class="sm:flex sm:items-center">
      <div
        ...
        class="flex items-center self-start justify-center flex-shrink-0 w-12 h-12 mx-auto rounded-full sm:mx-0 sm:h-10 sm:w-10"
      >
        <Icon
          icon="mdi-check"
          v-if="type === 'success'"
      ...
      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
        <h3
          v-if="title"
          class="mb-1 text-lg font-medium leading-6 text-gray-900 dark:text-gray-300"
          id="modal-headline"
        >
          {{ title }}
        </h3>
...
async function action(type: 'primary' | 'secondary') {
  ...
  if (type === 'primary' && props.primary) props.primary.action()
  if (type === 'secondary' && props.secondary) props.secondary.action()
}
